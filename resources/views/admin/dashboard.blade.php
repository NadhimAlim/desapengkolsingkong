        ...
        <div class="panel"
            ...
            <div class="simple-list" style="display: flex; flex-direction: column; gap: 4px;">
                @forelse ($recentMembers ?? [] as $member)
                    <div
                        style="display: flex; justify-content: space-between; align-items: center; padding: 14px 8px; border-bottom: 1px solid #f3f4f6; transition: background-color 0.2s;">

                        <div>
                            <strong
                                style="display: block; color: #1f2937; font-size: 1.05rem; margin-bottom: 4px;">{{ $member->name }}</strong>
                            <span class="muted-text" style="font-size: 0.85rem; color: #6b7280;">
                                Nama Usaha: <span
                                    style="color: #4b5563; font-weight: 600;">{{ $member->business_name }}</span>
                                @if ($member->phone_number)
                                    <span style="color: #d1d5db; margin: 0 6px;">|</span> WA: <span
                                        style="color: #10b981; font-weight: 600;">{{ $member->phone_number }}</span>
                                @endif
                            </span>
                        </div>
            ...
            @if ($recentMembers->hasPages())
                <div
                    style="margin-top: 24px; display: flex; justify-content: space-between; align-items: center; padding-top: 18px; border-top: 1px solid #f3f4f6;">
                    <div style="font-size: 0.875rem; color: #6b7280;">
                        Menampilkan <span
                            style="font-weight: 600; color: #1f2937;">{{ $recentMembers->firstItem() }}</span> - <span
                            style="font-weight: 600; color: #1f2937;">{{ $recentMembers->lastItem() }}</span> dari <span
                            style="font-weight: 600; color: #1f2937;">{{ $recentMembers->total() }}</span> Anggota
                    </div>
                    ...
                    <div style="display: flex; gap: 8px;">
                        ...
                        @if ($recentMembers->onFirstPage())
                            <span
                                style="padding: 8px 14px; background-color: #f3f4f6; color: #9ca3af; border-radius: 6px; font-size: 0.85rem; font-weight: 600; cursor: not-allowed; border: 1px solid #e5e7eb;">Sebelumnya</span>
                        @else
                            <a href="{{ $recentMembers->previousPageUrl() }}"
                                style="padding: 8px 14px; background-color: #ffffff; color: #374151; border: 1px solid #d1d5db; border-radius: 6px; font-size: 0.85rem; font-weight: 600; text-decoration: none; transition: background-color 0.2s;">Sebelumnya</a>
                        @endif

                        {{-- Tombol Selanjutnya --}}
                        @if ($recentMembers->hasMorePages())
                            <a href="{{ $recentMembers->nextPageUrl() }}"
                                style="padding: 8px 14px; background-color: #ffffff; color: #374151; border: 1px solid #d1d5db; border-radius: 6px; font-size: 0.85rem; font-weight: 600; text-decoration: none; transition: background-color 0.2s;">Selanjutnya</a>
                        @else
                            <span
                                style="padding: 8px 14px; background-color: #f3f4f6; color: #9ca3af; border-radius: 6px; font-size: 0.85rem; font-weight: 600; cursor: not-allowed; border: 1px solid #e5e7eb;">Selanjutnya</span>
                        @endif
                    </div>
                </div>
            @endif
